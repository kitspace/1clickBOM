...
let ninja = ninjaBuildGen('1.5.1', 'build/');
ninja.header(`#generated from ${path.basename(module.filename)}`);

let presets = "--presets es2015-script";
let browserify = `browserify -x $exclude --debug --transform [ babelify ${presets} ]`;
...
let sourceFiles = browser => sourceJs(browser);

let copyFiles = browser =>
    globule.find([
        `src/${browser}/html/*`,
        `src/${browser}/images/*`,
...
let browserifyEdge = function(target, browser, layer, exclude='') {
    ninja.edge(target)
        .from(`build/.temp-${browser}/${layer}.js`)
        .assign('exclude', exclude)
        .after(sourceFiles(browser).map(f => `build/.temp-${browser}/${path.basename(f)}`))
        .using('browserify');
    return targets[browser].push(target);
};


let iterable = ['main', 'popup', 'options', 'kitnic'];
...
let firefox_js = [];
let iterable1 = sourceJs('firefox');
for (let j = 0; j < iterable1.length; j++) {
    let file = iterable1[j];
    var target = `build/firefox/lib/${path.basename(file)}`;
...
for (var browser in targets) {
    var list = targets[browser];
    let iterable2 = sourceFiles(browser);
    for (let k = 0; k < iterable2.length; k++) {
        var f = iterable2[k];
        var target = `build/.temp-${browser}/${path.basename(f)}`;
...
let qunitSrc = 'build/.temp-chrome/qunit-1.11.0.js';
...
let iterable3 = copyFiles('chrome');
for (let i1 = 0; i1 < iterable3.length; i1++) {
    var f = iterable3[i1];
    var target = f.replace(/src\/.*?\//, "build/chrome/");
    ninja.edge(target).from(f).using('copy');
    targets.chrome.push(target);
}


let iterable4 = copyFiles('firefox');
for (let j1 = 0; j1 < iterable4.length; j1++) {
    var f = iterable4[j1];
    var target = f.replace(/src\/.*?\//, "build/firefox/data/");
    ninja.edge(target).from(f).using('copy');
    targets.firefox.push(target);
}
...
let manifest = 'build/chrome/manifest.json';
...
let chrome_package_name = `1clickBOM-v${version}-chrome`;
...
let firefox_package = `build/1clickBOM-v${version}-firefox.xpi`;

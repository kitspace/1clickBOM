...
class @RS extends RetailerInterface
    ...
    addItems: (items, callback) ->
        if /web\/ca/.test(@cart)
            @_get_adding_viewstate_web_ca (that, viewstate, form_id) ->
                that._add_items_web_ca(items, viewstate, form_id, callback)

    _add_items_web_ca: (items, viewstate, form_id , callback) ->
            url = "http" + @site + @cart 
            ...
            #        console.log(event)
            #xhr.send(params)
    _get_adding_viewstate_web_ca: (callback)->
        that = this
        url = "http" + @site + @cart
        xhr = new XMLHttpRequest

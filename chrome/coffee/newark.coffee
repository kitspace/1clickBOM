...
class @Newark extends RetailerInterface
    ...
    addItems: (items, callback) ->
        ...
        post url, params, (event) ->
            doc = DOM.parse(event.target.responseText)
            success = doc.querySelector("#formErrors").className != ""
            if not success
                ...
                fails = []
                retry_items = []
                regex = /The following Newark part number is invalid: (.*). Please revise your selection./g
                result = regex.exec(event.target.responseText)
                while result != null
                    fail_names.push(result[1])
                    result = regex.exec(event.target.responseText)
                for item in items

...
function save_options() {
    ...
    if (settings_data[country]) {
        ...
        for (const retailer in settings_data[country]) {
            const checked = document.querySelector(`input[name=${retailer}]:checked`)
            if (checked) {
    ...
    browser.prefsGet(['settings', 'country'], function(stored) {
        ...
        return (() => {
            ...
            for (const retailer in settings_data[stored.country]) {
                const choices = settings_data[stored.country][retailer].site.options
                const _default = settings_data[stored.country][retailer].site.value
                ...
                if ((stored.settings != null) && (stored.settings[stored.country] != null) && (Boolean(Object.keys(stored.settings[stored.country]).length))) {
                    const id = `id_${stored.settings[stored.country][retailer].site}`
